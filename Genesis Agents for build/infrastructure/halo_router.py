...
class HALORouter:
    ...
    def __init__(
        ...
        # MCP Client integration (Integration #76): Universal tool access protocol
        self.mcp_client = None
        self.mcp_enabled = os.getenv('ENABLE_MCP', 'false').lower() == 'true'
        if self.mcp_enabled:
            try:
                from infrastructure.mcp_client import initialize_mcp_from_env
                # Initialize asynchronously in background (will be ready for use)
                import asyncio
                try:
                    ...
                    if loop.is_running():
                        ...
                        # Run synchronously
                        # CRITICAL FIX: Pass user_id and agent_role for multi-tenant isolation
                        user_id = getattr(self, 'router_id', None) or f"halo-{id(self)}"
                        agent_role = "HALORouter"
                        self.mcp_client = loop.run_until_complete(initialize_mcp_from_env(
                            user_id=user_id,
                    ...
                    pass
                logger.info("MCP client integration enabled for universal tool access")
            except Exception as e:
    ...
    async def _initialize_mcp(self) -> None:
        """Initialize MCP client asynchronously (Integration #76)"""
        try:
            from infrastructure.mcp_client import initialize_mcp_from_env
            # CRITICAL FIX: Pass user_id and agent_role for multi-tenant isolation
            # HALO router doesn't have a specific user, so use router instance ID
            user_id = getattr(self, 'router_id', None) or f"halo-{id(self)}"
            agent_role = "HALORouter"
            self.mcp_client = await initialize_mcp_from_env(
                user_id=user_id,
                agent_role=agent_role
            )
            logger.info(f"✅ MCP client initialized with {len(self.mcp_client.get_available_tools())} tools")
        except Exception as e:
