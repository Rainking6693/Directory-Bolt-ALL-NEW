...
def validate_generated_code(code: str) -> Tuple[bool, str]:
    ...
    dangerous_imports = [
        ...
        # MEDIUM PRIORITY FIX: Add pickle, marshal, shelve to dangerous imports
        'pickle', 'marshal', 'shelve', 'pickletools', 'dbm'
    ]
...
def redact_credentials(text: str, use_llm_detection: bool = False) -> str:
    """
    Redact common credential patterns from text.

    ISSUE #10 FIX: Prevents credential leakage in trajectory metadata
    MEDIUM PRIORITY FIX: Extended with LLM-based detection for non-standard formats
    ...
    - Bearer tokens
    - AWS keys (AKIA...)
    - LLM-based detection for non-standard formats (if enabled)

    Args:
        text: Text potentially containing credentials
        use_llm_detection: If True, use LLM to detect non-standard credential formats
    ...
    # MEDIUM PRIORITY FIX: LLM-based detection for non-standard formats
    if use_llm_detection:
        try:
            ...
            # Use LLM to detect credentials that don't match regex patterns
            detection_prompt = f"""Analyze this text for credentials or secrets that should be redacted:

{text}

Identify any:
- API keys or tokens (even if not in standard format)
...
{{
    ...
    "redacted_text": "text with credentials replaced by [REDACTED]"
}}"""

            # Note: This is async, so in practice would need async context
            # For now, this is a placeholder for the LLM-based detection
            logger.debug("LLM-based credential detection requested (async implementation needed)")
        except Exception as e:
            logger.warning(f"LLM-based credential detection failed: {e}. Using regex fallback.")
